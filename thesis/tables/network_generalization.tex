\begin{table}[ht]
    ...
    \begin{tabular}{llcccc}
        \hline
        Configuration & Schedule & $C$ & $F$ & Rate & Feasible \\
        \hline
    ...
    \caption[Optimizer vs.\ paper schedule under non-ideal networks]{Optimizer versus the paper schedule under two non-ideal network configurations. Both use $N=10$, $e_d=0.01$, $E_{\max}=100$, $\gamma=0$, and non-zero inner-qubit error $p_X^{\mathrm{in}}=p_Z^{\mathrm{in}}=3\times10^{-3}$, unlike the source paper's idealized inner-qubit assumption. The configurations differ in the number of redundant \Gls{bsm} arms per hop. A schedule is feasible when $F\ge F_{\min}=0.9$.}
    \label{tab:netgen}
\end{table}