...
\begin{figure*}[ht]
    ...
    \end{subfigure}
    \caption{Rate and fidelity vs.\ depolarizing error $e_d$ at $N=10$, $E_{\max}=100$, for the paper baseline, matched-cost optimizer, and budget-relaxed optimizer. The paper baseline's higher fidelity (right) reflects unused surplus above $F_{\min}=0.9$, purchased with cost and rate the optimizer reallocates instead.}
    \label{fig:edsweep}
\end{figure*}
\FloatBarrier
...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=0.62\textwidth]{figures/results/min_budget_vs_n.png}
