...
Section~\ref{sec:exp:validation} reports the evaluator's agreement with the source paper's Fig.\,5 at its single published headline point, $e_d=0.01$. Table~\ref{tab:app:validation} give the evaluator's fidelity output for the raw, heralded-pumping, and optimistic-pumping canonical schedules (Section~\ref{sec:bg:decoherence}) across the full $e_d \in [0, 0.01]$ grid used throughout Chapter~\ref{chap:results}, rather than only at that one point.
...
% \centering
% \captionof{table}{Evaluator-derived fidelity vs.\ $e_d$ for the three canonical schedules of Section~\ref{sec:bg:decoherence}, at $N=10$. Only $e_d=0.000$ and $e_d=0.010$ have a published reference value to compare against (see text).}
% \label{tab:app:validation}
% \small
% \begin{tabular}{cccc}
% \hline
% $e_d$ & $F_{\text{raw}}$ & $F_{\text{baseline}}$ & $F_{\text{flexible}}$ \\
% \hline
% 0.000 & 1.0000 & 1.0000 & 1.0000 \\
% 0.001 & 0.9803 & 0.9932 & 0.9933 \\
% 0.002 & 0.9610 & 0.9861 & 0.9866 \\
...
% \begin{minipage}[c]{0.48\textwidth}
% \centering
% \includegraphics[width=\textwidth]{figures/appendix/fidelity_vs_ed_validation.png}
% \captionof{figure}{Same data as Table~\ref{tab:app:validation}, plotted.}
