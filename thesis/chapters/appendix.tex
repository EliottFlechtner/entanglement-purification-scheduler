...
\TOFIX{populate once figures are designed}{This section is reserved for illustrative figures supporting Chapter~\ref{chap:relatedwork} (e.g.\ a labeled diagram of the \Gls{rgs}/\Gls{hrgs} structure, or a schematic comparing first-, second-, and third-generation repeater protocols like the one in~\cite{Muralidharan2016Efficient}). None of the related-work discussion in Chapter~\ref{chap:relatedwork} currently depends on a figure placed here; add the figure, a short caption, and a forward pointer from the relevant paragraph of Chapter~\ref{chap:relatedwork} to this section once the figures are designed.}
...
Table~\ref{tab:params} summarizes every parameter varied anywhere in Chapter~\ref{chap:experiments}, alongside the reference value used whenever it is held fixed, and the section in which it is swept. It is referenced throughout Section~\ref{sec:exp:params} and is placed first here since later sections in this appendix assume it as a common reference point.
...
These diagnostics justify two implementation choices used throughout Chapter~\ref{chap:experiments}: the default beam width $w_{\text{beam}}=25$ (Section~\ref{sec:alg:beam}) and the claim that beam search scales comparably to the exact dynamic program once same-span pumping is enabled (Section~\ref{sec:alg:pumping}).
