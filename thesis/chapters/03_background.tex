...
The Bridging paper's own contribution is the \emph{half repeater graph state} (\Gls{hrgs}): half of an \Gls{rgs}, held at a single station rather than requiring the full $2m$-qubit state to exist in one place (Figure~\ref{fig:bg:hrgs-combo}). Two \Glspl{hrgs} are combined into a full \Gls{rgs} by the operation shown in the same figure -- called \emph{Swap} in this thesis -- and treated as a single operation throughout (Section~\ref{sec:bg:backbone}). Because a \Gls{hrgs} only needs to be generated and briefly held, an end node can act as its own source: it generates a \Gls{hrgs} using its stationary anchor and emits the photonic qubits toward its neighbouring \Gls{absa}, rather than requiring a separate \Gls{rgss}. This is what lets end-node memory scale additively with the number of hops instead of multiplicatively, the central resource-saving claim of the Bridging paper~\cite{Benchasattabuse2026Bridging}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{figures/background/HRGS_combo.png}
    \caption{\TOFIX{fix temporary text in figure, refer to a b in caption, adjust CZ/MXMX in figure}{\emph{Left}: an isolated \Gls{hrgs}, generated and held by a single station (an \Gls{rgss}, or an end node acting as its own source); its anchor qubit is the surviving qubit after the swap on the right, while its tree-encoded inner qubits and outer qubits follow the same labelling as Figure~\ref{fig:rw:rgs-combo}. \emph{Right}: two such \Glspl{hrgs}, generated independently and possibly at different stations, are combined into one full \Gls{rgs} by a controlled-phase ($CZ$) gate between their two anchor qubits followed by a joint $XX$ measurement on those same qubits. The outcome of this measurement realizes entanglement swapping between the two \Glspl{hrgs}' remaining qubits, the same effect a \Gls{bsm} (Figure~\ref{fig:bsm}) has on two independent Bell pairs, but applied here to two graph-state anchors instead.}}
...
\begin{figure}[htb]
    ...
    \caption{An $N$-hop network between Alice (station~0) and Bob (station~$N$). Interior stations $1,\dots,N-1$ are \Gls{rgss} nodes, each generating a full \Gls{rgs} and sending one half toward each neighbouring \Gls{absa}; the two end nodes instead generate only the single \Gls{hrgs} each needs (Section~\ref{sec:bg:hrgs}). Every hop's \Gls{absa} receives outer qubits from both neighbouring stations and performs the corresponding \Glspl{bsm} (grey crossing lines); a successful \Gls{bsm} extends the established entanglement by one more hop, giving rise to the notion of \emph{span} introduced next.}
...
\TOFIX{reread}{Crucially, \emph{optimistic} is not a property of an individual purification operation. It follows from the placement of heralding steps relative to purification steps in the complete schedule. Section~\ref{sec:model:dag} states this criterion precisely in the formal \Gls{dag} representation.}