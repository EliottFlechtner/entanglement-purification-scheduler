...
\TODO{for all figures, recheck that caption isn't restating too much the main text. if restating, cut from the main text and replace with a shorter, less descriptive sentence and make the caption longer \& more detailled}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.5\textwidth]{figures/background/BSM_schematic.png}
%     \caption{Schematic of a Bell-state measurement (BSM). Two incoming photons/qubits enter the BSM, undergo a joint XX-type measurement, and the possible outcomes determine the resulting surviving entanglement.}
%     \label{fig:bsm}
% \end{figure}
...
A \emph{repeater graph state} (\Gls{rgs}) is one such graph state, with $2m$ inner qubits and $2m$ outer qubits, where $m$ is the number of outer-qubit arms on each side. It is proposed as the basis for a memory-free all-photonic quantum repeater~\cite{Azuma2015All}. The inner qubits form a complete bipartite (\emph{biclique}) graph and are tree-encoded~\cite{Varnava2006Loss}, allowing the logical measurements required by the protocol to be performed despite photon loss~\cite{Varnava2008How}, illustrated in Figure~\ref{fig:bg:rgs-structure}. Each outer qubit is attached to one inner qubit and is consumed by a \Gls{bsm} with a neighbouring station. Such a network has two station types: \Gls{rgss} nodes, which generate the \Gls{rgs} and hold it only until it is transmitted, and \Gls{absa} nodes, which perform the \Glspl{bsm} between neighbouring stations and forward only the resulting entanglement, never generating anything themselves~\cite{Azuma2015All,Benchasattabuse2026Bridging}. Figure~\ref{fig:bg:nhop-network} below shows how these two station types alternate along a full chain.

\glsreset{rgs}
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{figures/background/RGS_combo.png}
    \caption{Structure of a \Gls{rgs} with $m$ arms on each side. The inner qubits (yellow) form the two partitions of the \Gls{rgs} and are connected across partitions, while each inner qubit is linked to an outer qubit (red), which is used to establish entanglement with a neighbouring station. The inner qubits are tree-encoded to improve the robustness of their logical measurements against photon loss~\cite{Varnava2006Loss}. The right panel illustrates the corresponding tree encoding for branching vector $\vec b=(3,2,1)$, showing the physical qubits across successive tree levels.}
    \label{fig:bg:rgs-structure}
\end{figure}

The Bridging paper's own contribution is the \Gls{hrgs}: half of an \Gls{rgs}, consisting of an \emph{anchor qubit} together with its associated inner and outer qubits, generated and held at a single station rather than requiring the full $2m$-qubit state to exist in one place~\cite{Benchasattabuse2026Bridging} (\TOFIX{fix ref}{Figure~\ref{fig:bg:hrgs-combo}}). Two \Glspl{hrgs} are combined into a full \Gls{rgs} by a controlled-phase gate followed by an $XX$ measurement on their two anchor qubits, shown in \TOFIX{fix ref}{Figure~\ref{fig:bg:hrgs-combo}}. This realizes entanglement swapping -- called \emph{Swap} in this thesis -- and is treated as a single operation throughout (Section~\ref{sec:bg:backbone}). Because a \Gls{hrgs} only needs to be generated and briefly held, an end node can act as its own source: it generates a \Gls{hrgs} using its stationary anchor and emits the photonic qubits toward its neighbouring \Gls{absa}, rather than requiring a separate \Gls{rgss}. This is what lets end-node memory scale additively with the number of hops instead of multiplicatively, the central resource-saving claim of the Bridging paper~\cite{Benchasattabuse2026Bridging}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{figures/background/HRGS_combo.png}
    \caption{\TOFIX{extend a little bit}{An isolated \Gls{hrgs} is shown on the left, with its anchor qubit and associated inner and outer qubits. On the right, two \Glspl{hrgs} are combined into a full \Gls{rgs} via a controlled-phase ($CZ$) gate and an $XX$ measurement on the anchor qubits.}}
    \label{fig:bg:hrgs-combo}
\end{figure}

An $N$-hop chain between two end nodes has $N+1$ stations numbered $0,\dots,N$: stations $0$ and $N$ are the end nodes (Alice and Bob), and stations $1,\dots,N-1$ are \Gls{rgss} nodes. Every \Gls{rgss} generates a full \Gls{rgs} and distributes its two halves to the \Gls{absa} on each side; an end node generates only the one \Gls{hrgs} it needs and sends its photonic qubits to its single neighbouring \Gls{absa}. Each hop $i$, the link between stations $i-1$ and $i$, has one \Gls{absa} that receives outer qubits from both sides and performs the requisite \Glspl{bsm}, so that entanglement is established one hop at a time until it spans the whole chain~\cite{Benchasattabuse2026Bridging} (Figure~\ref{fig:bg:nhop-network}).

\begin{figure}[htb]
    \centering
    \includegraphics[width=\textwidth]{figures/background/Nhop_network.png}
    \caption{\TOFIX{extend a little bit}{An $N$-hop network between Alice (station 0) and Bob (station $N$). Each hop's \Gls{absa} receives outer qubits from the \Gls{hrgs}/\Gls{rgs} generated at its two neighbouring stations and performs the corresponding \Glspl{bsm} (grey crossing lines).}}
    \label{fig:bg:nhop-network}
\end{figure}

% FIGURE 4: N-hop network / resource progression
% Show an example of N-hop chain with Alice, intermediate RGSS and
...
\TODO{Design a conceptual purification figure for this section (not yet designed -- do not treat the block below as final). It should show purification as a $2\to1$ transformation (two lower-fidelity Bell pairs consumed, one higher-fidelity Bell pair produced), not reproduce circuit-level detail, and should situate the operation within the \Gls{rgs}/\Gls{hrgs} architecture of Section~\ref{sec:bg:hrgs} (e.g.\ two same-span resources between a pair of stations being purified into one). The circuit-level diagrams for $\mathcal C_\mathrm{YY}, \mathcal C_\mathrm{ZX}, \mathcal C_\mathrm{XZ}$ (Figure placeholder in Section~\ref{sec:bg:circuits} below) are kept separate from this conceptual figure and are deferred to Appendix~\ref{app:relatedwork-figures}/the unrestricted thesis version.}

% \begin{figure}[tb]
%     \centering
%     \includegraphics[width=0.6\textwidth]{figures/background/purification_concept.png}
%     \caption{Purification as a $2\to1$ transformation: two resources of equal span $\kappa$ (Section~\ref{sec:bg:backbone}) and lower fidelity are consumed by a probabilistic LOCC measurement, producing one resource of the same span with higher fidelity on success.}
%     \label{fig:bg:purification-concept}
% \end{figure}
...
\TODO{figure 5: purification circuits (cf commented desc) -- reserved for Appendix~\ref{app:relatedwork-figures} or the unrestricted thesis version, not the main text (Section~\ref{sec:bg:purification} intro above)}
...
% \begin{figure}[ht]
%     \centering
%     \includegraphics[width=\textwidth]{figures/background/entanglement_estab_SbS.png}
%     \caption{Step-by-step execution of entanglement swapping across a repeater chain. Each intermediate node performs a Bell-state measurement (BSM) on its local qubits, extending entanglement between the end nodes.}
%     \label{fig:entanglement-swapping}
% \end{figure}