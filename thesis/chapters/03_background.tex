...
\noindent Its quality is measured by a single scalar, the fidelity $F = \bra{\Phi^+}\rho\ket{\Phi^+}$, which quantifies how close the shared state $\rho$ is to a perfect pair ($F_\text{max}=1$). Depolarising noise on the transmitted qubit as well as memory decoherence cause $F$ to decrease, while photon loss instead reduces the probability of establishing a pair in the first place. Chapter~\ref{chap:model} evaluates each finished schedule by its fidelity, rate, and resource cost for producing such a pair between the end nodes. Improving one of these quantities is meaningful only relative to this definition of $F$.

\glsreset{bsm}

While Chapter~\ref{chap:relatedwork} surveys why direct photonic transmission cannot distribute such pairs over long distances, we introduce here the elementary operation used to establish and extend links: a two-qubit \Gls{bsm}. A \Gls{bsm} performs a joint measurement on two incoming photons and identifies their Bell state through the measurement outcomes.  On success, it effectively realizes entanglement swapping, extending the entanglement chain the network aims to create. Figure~\ref{fig:bsm} (Appendix~\ref{app:figures}) illustrates this process. Using passive linear optics and no ancillary photons, a \Gls{bsm} succeeds with probability at most 50\% per attempt, conditional on the photons arriving~\cite{Ewert20143,Benchasattabuse2026Bridging}. This ceiling is why the architecture below builds redundancy into every hop.
...
A \emph{repeater graph state} (\Gls{rgs}) is one such graph state, with $2m$ inner qubits and $2m$ outer qubits, where $m$ is the number of outer-qubit arms on each side, as illustrated in Figure~\ref{fig:rw:rgs-combo}. It is proposed as the basis for a memory-free all-photonic quantum repeater~\cite{Azuma2015All}. The inner qubits form a complete bipartite (\emph{biclique}) graph and are tree-encoded~\cite{Varnava2006Loss}, allowing the logical measurements required by the protocol to be performed despite photon loss~\cite{Varnava2008How}. Each outer qubit is attached to one inner qubit and is consumed by a \Gls{bsm} with a neighbouring station. Such a network has two station types: \Gls{rgss} nodes, which generate the \Gls{rgs} and hold it only until it is transmitted, and \Gls{absa} nodes, which perform the \Glspl{bsm} between neighbouring stations and forward only the resulting entanglement~\cite{Azuma2015All,Benchasattabuse2026Bridging}. Figure~\ref{fig:bg:nhop-network} below shows how these two station types alternate along a full chain.

The Bridging paper introduces the \emph{half repeater graph state} (\Gls{hrgs}), consisting of half of an \Gls{rgs} and held at a single station, avoiding the need to generate the full $2m$-qubit state at once (Figure~\ref{fig:bg:hrgs-combo}). Two \Glspl{hrgs} are combined into a full \Gls{rgs} through entanglement swapping, referred to as \emph{Swap} hereafter and treated as a single operation throughout (Section~\ref{sec:bg:backbone}). Because a \Gls{hrgs} only needs to be generated and briefly held, an end node can act as its own source: it generates a \Gls{hrgs} using its stationary anchor and emits the photonic qubits toward its neighbouring \Gls{absa}, rather than requiring a separate \Gls{rgss}. This is what lets end-node memory scale additively with the number of hops instead of multiplicatively, the central resource-saving claim of the Bridging paper~\cite{Benchasattabuse2026Bridging}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{figures/background/HRGS_combo.png}
    \caption[Combining two HRGS halves into a full RGS]{\textbf{(a)} An isolated \Gls{hrgs}, generated and held by a single station (an \Gls{rgss}, or an end node acting as its own source). \textbf{(b)} Two such \Glspl{hrgs}, generated independently and possibly at different stations, combined into one full \Gls{rgs} by a controlled-phase ($CZ$) gate between their anchor qubits followed by a joint $XX$ measurement on those same qubits. The measurement realizes entanglement swapping between the two \Glspl{hrgs}' remaining qubits, analogous to the effect of a \Gls{bsm} (Figure~\ref{fig:bsm}) on two independent Bell pairs, but applied here to graph-state anchors.}
    \label{fig:bg:hrgs-combo}
\end{figure}

An $N$-hop chain between two end nodes has $N+1$ stations numbered $0,\dots,N$: stations $0$ and $N$ are the end nodes (Alice and Bob), and stations $1,\dots,N-1$ are \Gls{rgss} nodes. Every \Gls{rgss} generates a full \Gls{rgs} and distributes its two halves to the \Gls{absa} on each side; an end node generates only the one \Gls{hrgs} it needs and sends its photonic qubits to its single neighbouring \Gls{absa}. Each hop $i$, the link between stations $i-1$ and $i$, has one \Gls{absa} that receives outer qubits from both sides and performs the required \Glspl{bsm}, establishing entanglement one hop at a time until it spans the whole chain~\cite{Benchasattabuse2026Bridging} (Figure~\ref{fig:bg:nhop-network}).

\begin{figure}[htb]
    \centering
    \includegraphics[width=\textwidth]{figures/background/Nhop_network.png}
    \caption[An $N$-hop repeater network]{An $N$-hop network between Alice (station~0) and Bob (station~$N$). Interior stations $1,\dots,N-1$ are \Gls{rgss} nodes, each generating a full \Gls{rgs} and sending one half toward each neighbouring \Gls{absa}; the two end nodes instead generate only the single \Gls{hrgs} each needs (Section~\ref{sec:bg:hrgs}). Every hop's \Gls{absa} receives outer qubits from both neighbouring stations and performs the corresponding \Glspl{bsm}; a successful \Gls{bsm} extends the established entanglement by one more hop, giving rise to the notion of \emph{span} introduced next (Section~\ref{sec:bg:backbone}).}
...
Because the \Gls{bsm}'s success probability is capped at 50\% (Section~\ref{sec:bg:bellpairs}), each hop provisions redundant outer-qubit arms rather than a single pair. Chapter~\ref{chap:model} uses $k_i$ to denote the arm count at hop $i$. At every \Gls{absa}, once one arm's \Gls{bsm} succeeds, that arm's inner qubits are \emph{kept}: the station \emph{discards} the redundancy by measuring every other arm's inner qubits in the $Z$ basis~\cite{Azuma2015All,Benchasattabuse2026Bridging}. This \emph{backbone selection} is forced by the protocol at every hop, on every trial. Figure~\ref{fig:app:backbone-selection} in Appendix~\ref{app:figures} illustrates this process using a $N=3$-hop example.
...
This parity bookkeeping is separate from the error vector: $s$ records the accumulated Pauli corrections associated with the measurement outcomes, while $\mathbf{e}$ describes the resource's noise. For each arm, the individual measurement outcomes can be combined into their \Gls{xor} parity, so only this single parity value needs to be communicated, regardless of the depth of the tree encoding~\cite{Benchasattabuse2026Bridging}. The schedule evaluator tracks these parities to determine the correct Pauli frame, while candidate schedules are compared using their error vectors and the resulting cost functions.
...
Here, \emph{optimistic} refers to when heralding is performed relative to purification in the schedule, rather than to a particular purification operation. The formal \Gls{dag} representation in Section~\ref{sec:model:dag} makes this distinction explicit.