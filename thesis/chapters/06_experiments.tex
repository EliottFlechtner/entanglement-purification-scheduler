...
Throughout this chapter and the rest of this thesis, \emph{the optimizer} refers to the software implementing one of the three search tiers of Chapter~\ref{chap:algorithms} that produced a given result, rather than to the schedule it returns.
...
A five-hop network with one hop assigned a $12\times$ higher inner-qubit error rate than its neighbours illustrates the benefit of allocating purification resources unevenly across a heterogeneous chain, detailed in Table~\ref{tab:weaklink} (Appendix~\ref{app:weaklink}).

The same tendency holds over 100 independently generated random $N=6$ networks (seeds 0--99, $e_d=0.01$, $E_{\max}=60$, $F_{\min}=0.9$), reported in full in Appendix~\ref{app:weaklink}: among the 100 networks, 80 have a feasible uniform link-level candidate; among those, the optimizer improves rate by a mean of $20.37\%$ and never performs worse in the tested sample (Section~\ref{sec:disc:synthesis}).
...
Section~\ref{sec:exp:excluded} (Appendix~\ref{app:results}) additionally reports a related optimality-scope check at $N=18$, where a hand-constructed schedule using a same-span construction excluded from the search reaches the paper's own budget.
