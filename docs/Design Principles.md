...
## Important caveat discovered while producing this document (resolved)
...
- `outputs/headline_experiment_n10/`, `outputs/sweep_ed_n10/` (at `e_d=0.01`), `outputs/sweep_min_budget_vs_n/` (at `N=10`), `outputs/sweep_min_budget_vs_ed/` (at `e_d=0.01`), and `outputs/sweep_hop_count/` (at `N=10`) all report the budget-relaxed optimum at the paper's `N=10` config as **cost=50, F=0.9047, rate=6713.18**. `docs/Optimizer Status.md` and `docs/Repository State & Progress.md` both describe this as a currently "reproducible" result.
...
- This affects only the *budget-relaxed* ("unconstrained-within-budget") variant across the affected outputs -- the paper-baseline and matched-cost numbers (drawn from fixed families, unaffected by span-frontier pruning) still reproduce exactly.

**Resolution.** Two fixes were considered:

1. **Algorithmic fix (attempted, reverted).** Modified `_SpanPartitionSearch.frontier()` to skip the final union re-cap step specifically under `beam_search` mode, relying on `base_pruned` and `pump_candidates` each already being independently capped. Result: only *partially* restored the headline number (cost=52/rate=6655.76, still short of cost=50/rate=6713.18) because crowding also happens one recursion level deeper, at intermediate spans' own `base_pruned` selection, which competes against pump-enlarged left/right pools from narrower spans -- a full fix would need two parallel per-span frontiers (pure-lineage vs. pump-augmented), a much larger restructuring. Worse, it caused a **5x full-test-suite slowdown** (65-70s -> 352.93s) by letting frontier sizes compound multiplicatively across recursion depth, confirming the original beam cap's purpose. **Declared unsuccessful and reverted** (see the explanatory comment left in `src/hrgs_scheduler/search/dp.py` at the final re-cap site).
2. **Expose + pin (implemented).** Added a public `enable_pumping: bool = True` keyword parameter to both `dp_search()` and `beam_search()` (default preserves current, pumping-enabled behavior exactly; no performance or correctness impact measured on the default path). Setting `enable_pumping=False` exactly reproduces the historical cost=50/F=0.9046928819497215/rate=6713.17509739292 result bit-for-bit. This parameter has been threaded into `experiments/sweep_ed.py`, `experiments/sweep_hop_count.py`, `experiments/sweep_min_budget_vs_n.py`, `experiments/sweep_min_budget_vs_ed.py`, and `outputs/headline_experiment_n10/README.md`'s reproduce snippet, and all four/five affected cached outputs have been regenerated under `enable_pumping=False` and reconfirmed to match the historical numbers exactly. `docs/Optimizer Status.md` and `docs/Repository State & Progress.md` were updated with a note that reproducing this exact number now requires the pin.

New results produced in this document (items 7-9, all freshly run against the current code, with the current pumping-enabled default) are internally consistent with each other and correctly reflect *current* default behavior; they were never affected by this regression since they don't cite the specific pre-pumping `N=10` budget-relaxed number.
...
**Statement:** the paper's own resource-cost convention (`e_max = 10*N`, linear in `N`) is not the point at which the fidelity floor genuinely becomes tight -- the minimum budget this repo's searched families actually need scales as roughly $N^{1.6}$, i.e. faster than linear, over the tested range (N=10–28), and the paper's formula becomes strictly insufficient beyond approximately N≈27–28 at `e_d=0.01`.

**Evidence:**
- `outputs/sweep_min_budget_vs_n/README.md`: descriptive power-law fit over `N in {10,12,14,16,18,20,21,22,23,24,26,28}` (pumping-integrated, 12 points): $e_{max}^{min} \approx 1.369 \cdot N^{1.620}$. The *ratio* of minimum-feasible-budget to the paper's own linear budget rises monotonically with `N` (0.60x at N=10, up through 0.992x at N=26, to **1.011x at N=28**) — i.e. the paper's fixed linear formula becomes progressively less over-provisioned as `N` grows, and is first crossed at N=28.
- **N=28 crossover:** `min_feasible_e_max=283 > 10*28=280`. This is the first point where `beam_search`'s pumping-enabled families provably need *more* than the paper's own budget to clear the fidelity floor. The 10N formula derives from the paper's hardware model (5 half-RGS copies per side × 2 sides × N hops); the crossover makes its insufficiency concrete and quantifiable rather than merely extrapolated.
- **Fit history:** the original fit ($0.593 \cdot N^{1.909}$, over N in {10,12,14,16} only) was produced before pumping was integrated and excluded N=18 as "not found." The first pumping-integrated re-run (N=10–20) gave $0.995 \cdot N^{1.742}$. The current 12-point fit gives $1.369 \cdot N^{1.620}$. All three are super-linear; the exponent has been refining downward as more high-N points are added. Old numbers are preserved in [docs/archive/Roadmap_Derisk_and_Reframe_Results.md](archive/Roadmap_Derisk_and_Reframe_Results.md) §2.
- This finding is orthogonal to the reproducibility caveat above: it is about the *ratio and trend* across `N`, not the exact `N=10` rate value. The qualitative "ratio increases with N and eventually crosses 1" trend holds regardless of the exact per-point figure.
